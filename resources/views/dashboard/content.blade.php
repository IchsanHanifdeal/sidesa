...
<div class="flex flex-col max-sm:mt-4 max-lg:w-full w-[36rem] gap-4 overflow-y-auto">
  ...
  <form method="post" enctype="multipart/form-data" action="{{ route('posts.store') }}" class="w-full">
    ...
    <div class="flex gap-4 p-3 bg-white rounded-lg">
      ...
      @if (Auth::user()->image)
        <img class="flex-shrink-0 border rounded-full size-11" src="{{ asset('storage/images/' . Auth::user()->image) }}" alt="">
      @else
        <img class="flex-shrink-0 border rounded-full size-11" src="https://ui-avatars.com/api/?name={{ Auth::user()->name }}&color=7F9CF5&background=EBF4FF">
      @endif
      <div class="flex flex-wrap w-full">
        <textarea class="w-full p-2 px-4 text-black bg-gray-50 rounded- placeholder:text-[15px] rounded-lg" name="content" placeholder="Apa yang anda pikirkan?"></textarea>
          ...
          <div class="flex items-center gap-4">
            ...
            <div class="flex items-center gap-4">
              <div class="flex items-center gap-1">
                <input type="checkbox" id="for_sale" checked="checked" class="checkbox checkbox-secondary checkbox-xs" name="for_sale" />
      ...
      <div class="flex flex-col w-full gap-3 p-3 text-black bg-white rounded-lg">
        <div class="flex gap-3">
          @if ($post->creator_image)
            <img class="flex-shrink-0 border rounded-full size-9" src="{{ asset('storage/images/' . $post->creator_image) }}" alt="">
          @else
        ...
        @if ($post->photo)
          <img class="flex-shrink-0 border rounded-lg w-full max-h-[300px] object-cover" src="{{ asset('storage/images/' . $post->photo) }}" alt="">
        @endif
        ...
        <div class="flex justify-between w-full mt-2">
          <div class="flex gap-3">
            <button class="like-button {{ $post->is_liked ? 'text-red-500' : 'text-black' }} flex items-center gap-2 text-[15px] cursor-pointer"
              data-post-id="{{ $post->id }}">
              <x-lucide-heart class="flex-shrink-0 size-5 {{ $post->is_liked ? 'fill-red' : 'fill-white' }} {{ $post->is_liked ? '' : 'stroke-black' }}" />
          ...
          <div class="flex items-center space-x-2">
            ...
            @if ($post->for_sale)
              ...
              @endphp
              <a href="{{ $msg }}" target="_blank" class="text-center text-white bg-green-500 btn btn-xs">Hubungi Penjual</a>
            @endif
          </div>
        </div>
      </div>
